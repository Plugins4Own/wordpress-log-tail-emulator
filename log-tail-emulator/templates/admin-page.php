...
<div class="wrap">
    ...
    <div style="margin-bottom:10px;">
        <input type="text" id="lte-filter" placeholder="<?php echo esc_attr__('Filter logs (e.g., "error" or "PHP")', 'log-tail-emulator'); ?>"
               style="width:300px;padding:5px;border:1px solid #0f0;background:#111;color:#0f0;">

        <button id="lte-clear" style="margin-left:10px;padding:5px 10px;background:#333;color:#0f0;border:1px solid #0f0;cursor:pointer;">
            <?php echo esc_html__('Clear', 'log-tail-emulator'); ?>
        </button>

        <!-- NOWY PRZYCISK PAUSE / RESUME -->
        <button id="lte-pause" style="margin-left:15px;padding:5px 14px;background:#c33;color:#fff;border:none;border:none;border-radius:4px;cursor:pointer;font-weight:bold;">
            Pause
        </button>
    </div>
    ...
    <div style="margin-top:10px;font-size:12px;color:#666;">
        ...
        <strong><?php echo esc_html__('File size:', 'log-tail-emulator'); ?></strong>
        <span id="lte-file-size">
            <?php 
            $file_reader = new LTE_File_Reader();
            echo $file_reader->file_exists() 
                ? esc_html(size_format($file_reader->file_size())) 
                : esc_html__('File does not exist', 'log-tail-emulator');
            ?>
        </span>
